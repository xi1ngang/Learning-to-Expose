...
class Trainer:
    def __init__(self, options):
        ...
        # setup model
        self._setup_model(
            self.opt.crop_size, 2 * self.opt.slide_num + 1
        )  # state dim plus 1 due to the flicker state
    ...
    def _setup_model(self, n_observations, n_actions):
        """Setup model"""
        assert self.device
        ...
        self.policy_net = DQN(n_observations, n_actions, self.opt).to(self.device)
        self.target_net = DQN(n_observations, n_actions, self.opt).to(self.device)
    ...
    def _save_model(self, episode_num=None):
        ...
        path_suffix = f"trained_wbfactor/model_2100_nbd_{episode_num}_{self.opt.slide_num}_{self.opt.hidden_layer_size}.pt"
    ...
    def _save_rewards(self):
        ...
        rewards_path = f"{path}/trained_wbfactor/reward_2100_nbd_{self.opt.slide_num}_{self.opt.hidden_layer_size}.npy"
        with ManifoldClient.get_client(bucket) as client:
