...
class Validator:
    ...
    def _reward_plot(self, reward_path):
        ...
        plt.plot(reward_list, color="blue", label="Rewards")
        ...
        plt.savefig(
            "/data/sandcastle/boxes/fbsource/fbcode/pi_algo/projects/DRL_AEC/outputs/reward_wfactor_1400_nbd_plot.png"
        )
    ...
    def _model_eval_wflicker_inference(
        ...
        while steps2converge < max_steps:
            ...
            image = val_image_data_aug[curr_index]["image"]
            image_tensor = torch.tensor(image)
            state, feature = feature_extractor(image_tensor)
            # Prepare state tensor based on the number of frames
            state_tensor = (
                torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(device)
            )
        ...
        for scene_index in range(1, 16):
            ...
            for episode in episode_num:
                ...
                with ManifoldClient.get_client(bucket) as client:
                    ...
                    # )
                    client.sync_get(
                        f"{path}/trained_wbfactor3/model_2100_nbd_{episode}_40_64.pt",
                        buffer,
                    )
                ...
                model = DQN(n_observations, n_actions, self.opt)
            ...
            # with open(
            #     f"/data/sandcastle/boxes/fbsource/fbcode/pi_algo/projects/DRL_AEC/outputs/new_flicker_1400_model/results_flicker_{episode}_{scenario}_scene{scene_index}.json",
            #     "w",
            # ) as f:
    ...
    def evaluation(self):
        ...
        for scenario in scenarios:
            for scene_index in range(1, 6):
                ...
                # with open(
                #     f"/data/sandcastle/boxes/fbsource/fbcode/pi_algo/projects/DRL_AEC/outputs/results_old_model_{scenario}_scene{scene_index}.json",
                #     "w",
                # ) as f:
                #     json.dump(all_results, f, indent=4)
...
def main() -> None:
    ...
    # validator._reward_plot("tree/trained_wbfactor2/reward_2100_nbd_40_64.npy")
