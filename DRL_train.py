...
class Trainer:
    ...
    def _save_model(self, episode_num=None):
        ...
        if episode_num is None:
            # Save final model if no episode number provided
            save_name = "final_model"
        else:
            # Save checkpoint with episode number
            save_name = f"checkpoint_{episode_num}"

        # Create save dictionary with model state
        save_dict = {
        ...
        model_save_path = os.path.join(save_path, f"{save_name}.pt")
        torch.save(save_dict, model_save_path)

        # Add scheduler state if it exists
        if self.model_lr_scheduler is not None:
            save_dict['scheduler_state_dict'] = self.model_lr_scheduler.state_dict()
    ...
    def _save_rewards(self):
        ...
        save_path = os.path.join(self.opt.save_path, "rewards.pt")
        torch.save({
    ...
    def train(self):
        ...
        for num_epoch in range(self.opt.num_epochs + 1):
            if num_epoch % ((self.opt.num_epochs + 1) / 70) == 0:
                ...
                scene_path = f"ExpoSweep/training/{current_scenario}/{current_scenario}_scene{scene_index}"
                image_data = load_dataset(scene_path)
